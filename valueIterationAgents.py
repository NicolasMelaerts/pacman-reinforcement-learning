...
class ValueIterationAgent(ValueEstimationAgent):
  ...
  def __init__(self, mdp, discount = 0.9, iterations = 100):
    ...
    for _ in range(iterations):
      new_values = util.Counter()  # Pour stocker v_{k+1}
    
      for state in mdp.getStates():
          if not mdp.getPossibleActions(state):  # Cas où un état n'a pas d'actions disponibles dans le MDP
              new_values[state] = 0
          else:
              # Calculer max
              new_values[state] = max(
                  sum(prob * (mdp.getReward(state, action, next_state) + 
  ...
  def getPolicy(self, state):
    ...
    if not actions:  # Cas sans choix d'action
        return None
